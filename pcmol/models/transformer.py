...
class TransformerDecoder(nn.Module):
    ...
    """
    def __init__(self, vocab, d_model=1024, d_emb=384, n_heads=16, 
                 d_feedforward=2048, n_layers=12, **kwargs):
        ...
        blocks = [CustomDecoderBlock(d_model, d_emb, n_heads, d_feedforward=d_feedforward, **kwargs)
                  for _ in range(n_layers)]
        self.blocks = nn.ModuleList(blocks)
        self.token_projection = nn.Linear(d_model, self.vocab.size)

    def forward(self, x, af_emb, key_padding_mask=None, attn_mask=None, encoder_mask=None):
        """
        Args:
        ...
        for block in self.blocks:
            x = block(x, mem=af_emb, key_padding_mask=key_padding_mask,
                      attn_mask=attn_mask, encoder_mask=encoder_mask)
...
class AF2SmilesTransformer(nn.Module):
    ...
    """
    def __init__(self, vocab, d_emb=384, d_model=1024, n_heads=16, 
                 d_feedforward=2048, n_layers=16, dropout=0.1, padding_idx=0, 
                 dev='cuda', loss_coefficients={'generative': 1.0, 'pchembl': 0.5},
                 use_pooling=False, pcm=False,  pchembl_arch=[], **kwargs):
        ...
        self.decoder = TransformerDecoder(
            self.vocab, d_emb=d_emb, d_model=d_model, dropout=dropout,
            n_heads=n_heads, d_feedforward=d_feedforward, n_layers=n_layers,
            **kwargs)
    ...
    def forward(self, x, af_emb, train=False, pchembl_targets=None, calc_pchembl=False):
        ...
        if not train:
            ...
            for step in range(self.vocab.max_len-1):
                ...
                dec, z = self.decoder(
                    decoder_input.transpose(0, 1), af_emb=af_emb.transpose(0,1), 
        ...
        # Training
            ...
            proj, z = self.decoder(decoder_input.transpose(0, 1), af_emb=af_emb.transpose(0,1), 
                                   key_padding_mask=key_padding_mask, attn_mask=attn_mask,
                                   encoder_mask=encoder_mask)
