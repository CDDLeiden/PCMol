...
class PapyrusStandardizer:
    def __init__(
        ...
        self._settings = {
            ...
            "extra_organic_atoms" : sorted(extra_organic_atoms) if extra_organic_atoms else [],
    ...
    def fix_errors(self, mol, error):
        if error == StandardizationResult.MIXTURE_MOLECULE and self._settings["mixture_handling"] == "keep_largest":
    ...
    def convertSMILES(self, smiles, verbose=False):
        ...
        out = Papyrus_standardizer.standardize(
            ...
            filter_mixtures=False if self._settings["mixture_handling"] == "keep" else True,
            ...
            tautomer_allow_stereo_removal=self._settings["tautomer_allow_stereo_removal"]
        )
        ...
        if not StandardizationResult.CORRECT_MOLECULE in results:
            ...
            if not mol:
                ...
                return self.convertSMILES(Chem.MolToSmiles(mol, isomericSmiles=self._settings["keep_stereo"], canonical=self._settings["canonize"])), smiles
        else:
            return Chem.MolToSmiles(out[0], canonical=self._settings["canonize"], isomericSmiles=self._settings["keep_stereo"]) if out[0] else None, smiles
    ...
    def getID(self):
        ...
        return "PapyrusStandardizer~" + ":".join([f"{key}={str(self._settings[key])}" for key in sorted_keys])
